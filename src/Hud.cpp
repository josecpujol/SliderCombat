...
void Hud::drawHealthBar() {
  ...
  glBegin(GL_QUADS);
  glVertex2f(distance_from_left, screen_height - distance_from_bottom);
  glVertex2f(distance_from_left + length, screen_height - distance_from_bottom);
  glVertex2f(distance_from_left + length, screen_height - distance_from_bottom - bar_height);
  glVertex2f(distance_from_left, screen_height - distance_from_bottom - bar_height);
  glEnd();

  // Draw heath bar on top
  glColor3f(0.6f, 0.1f, 0.02f);
  int lenght_health = (health.value * length) / health.max_value;
  int reduction = 5;
  glBegin(GL_QUADS);
